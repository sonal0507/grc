...
// Pre-configured JIRA credentials (your account)
const PRECONFIG_CREDENTIALS = {
    baseUrl: 'https://oswalshobha31-1768193369440.atlassian.net',
    email: 'oswalshobha31@gmail.com',
    apiToken: 'ATATT3xFfGF0uSmVhO5H4E7Ayh7emB_7wsi9-E1gHDKkRXx__fIY_SlLF5OFFwMaVYzyNgSX_9oXGGycQoOim651YFPhdWf4RdAS54OPm1xCUpiBwDvfacmotnPsTe8Yfx_RmRm9P4d9NT9djnUS_fHpdw-nkXKZmNwkSv_cTYaCI_B-gs-u4fo=4F820661',
    projectKey: 'SCRUM'
};
...
window.JIRAIntegration = {
    ...
    loadConfig: function() {
        try {
            ...
            } else if (typeof PRECONFIG_CREDENTIALS !== 'undefined') {
                // Use pre-configured credentials if no stored config
    ...
    bindJIRAEvents: function() {
        ...
        if (this.config.baseUrl) {
            ...
            document.getElementById('jiraIssueType').value = this.config.issueType;
            document.getElementById('jiraAutoCreate').checked = this.config.autoCreate;
        }
    ...
    checkExistingGaps: function() {
        ...
        if (completedAssessments.length > 0) {
            document.getElementById('btnAnalyzeGaps').disabled = false;
            document.getElementById('gapCountBadge').textContent = 
                `${completedAssessments.length} assessment(s) with gaps`;
        }
    },
    ...
    analyzeGaps: function() {
        if (!inlineAssessmentState.currentFramework) {
            showNotification('Please complete an assessment first', 'warning');
            return;
        }

        const gaps = this.identifyGaps(inlineAssessmentState.currentFramework);
        ...
        if (gaps.length === 0) {
            ...
            `;
            document.getElementById('btnCreateTickets').disabled = true;
        } else {
            this.renderGapsList(gaps);
            document.getElementById('btnCreateTickets').disabled = false;
            document.getElementById('gapCountBadge').textContent = `${gaps.length} gaps found`;
        }
    },
    ...
    identifyGaps: function(frameworkCode) {
        ...
        const responses = inlineAssessmentState.responses || {};
        const gaps = [];
        ...
        assessmentData.sections?.forEach(section => {
            section.questions?.forEach(question => {
                ...
                // Only count as gaps if answer is explicitly 'no' or 'partial'
                // Unanswered questions (undefined) should NOT be counted as gaps
                if (answer === 'no' || answer === 'partial') {
                    ...
                    gaps.push({
                        ...
                        currentScore: answer === 'no' ? 0 : 50,
                        maxScore: 100,
                        severity: severity,
                        selectedOption: selectedOption?.label || (answer === 'no' ? 'Not Implemented' : 'Partially Implemented'),
                        selectedDesc: selectedOption?.desc || (answer === 'no' ? 'No implementation' : 'Partial implementation'),
    ...
    /**
     * Calculate gap severity based on score or answer
     */
    calculateSeverity: function(score) {
        // Handle string answers
        if (score === 'no') return 'High';
        if (score === 'partial') return 'Medium';
    ...
    createTickets: function() {
        ...
        // Close the modal
        const modal = bootstrap.Modal.getInstance(document.getElementById('createTicketsModal'));
        modal.hide();
    ...
    createTicketsBatch: function(gaps, index, createdTickets, modal) {
        ...
        fetch('/api/jira/create-tickets', {
            ...
            body: JSON.stringify({
                ...
                issueType: this.config.issueType,
    ...
    buildTicketPayload: function(gap) {
        ...
        return {
            ...
            customFields: {
                ...
                evidenceRequired: 'Policy documents, Implementation evidence, Audit reports',
                remediationSteps: '1. Review gap analysis, 2. Develop remediation plan, 3. Implement controls, 4. Gather evidence, 5. Update assessment'
            }
        };
    },
